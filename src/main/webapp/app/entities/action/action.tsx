...
export const Action = (props: IActionProps) => {
  ...
  const { actionList, match, totalItems } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        {actionList && actionList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  Incident <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {actionList.map((action, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={action.start} format={APP_DATE_FORMAT} />
                  </td>
                  <td>
                    <TextFormat type="date" value={action.end} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{action.incident ? <Link to={`incident/${action.incident.id}`}>{action.incident.title}</Link> : ''}</td>
          ...
          <div className="alert alert-warning">No Actions found</div>
        )}
      </div>
